...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('download_bkam_taux_csvs.log'),
...
def construct_url(date):
    ...
    base_url = "https://www.bkam.ma/export/blockcsv/2340/c3367fcefc5f524397748201aee5dab8/e1d6b9bbf87f86f8ba53e8518e882982"
...
def download_csv(url, date, output_dir, failed_dir, max_retries=3):
    ...
    try:
        ...
        # Tenter de lire le CSV pour validation
        try:
            df = pd.read_csv(
                temp_filename,
                sep=';',
            ...
            # Filtrer les lignes valides (date ou 'Total')
            df = df[df['Date d\'échéance'].str.match(r'\d{2}/\d{2}/\d{4}') | (df['Date d\'échéance'] == 'Total')]
            if df.empty:
                raise ValueError("Aucune donnée valide (dates ou 'Total') trouvée dans le CSV.")
            # Si valide, renommer
            date_str = date.strftime("%Y%m%d")
            csv_filename = os.path.join(output_dir, f"taux_{date_str}.csv")
...
def main():
    # Paramètres pour la période
    start_date = datetime(2020, 1, 1)
    end_date = datetime(2025, 5, 22)

    output_dir = "bkam_taux_csvs"
    failed_dir = "failed_downloads_taux"
    ...
    logger.info(f"{len(csv_files)} fichiers CSV téléchargés dans {output_dir}.")
